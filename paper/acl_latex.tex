...
On the validation set, we obtain $\geq$70\% F1-score for the majority of language-task pairs.
...
whereas the bottom performers are \textsc{cop} (0.849), \textsc{san} (0.839), and \textsc{hbo} (0.200).

\paragraph{Error analysis for \textsc{cop} and \textsc{hbo}.}
Our performance on \textsc{cop} and \textsc{hbo} are below random chance.
We suspect that it's due to our tokenization process during finetuning.
