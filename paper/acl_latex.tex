...
For languages with subtokens such as \textsc{cop} and \textsc{hbo}, we merged each token and used the full multi-word expression (MWE) during training.
\lj{show example}
...
Similar to the POS tagger, we merged each token for every multi-word expression (MWE) during training.
\lj{show example}
